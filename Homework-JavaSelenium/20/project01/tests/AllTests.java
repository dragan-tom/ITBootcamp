...
//        driver.navigate().refresh();
    ...
    public void goToLoginPage() {
        driver.get(excelReader.getStringData("URL", 2, 2));
    }

    public void loginWithCredentials() throws InterruptedException {
        goToLoginPage();
        loginPage.getUsernamePlaceholder().sendKeys(excelReader.getStringData("Login", 1, 0));
        loginPage.getPasswordPlaceholder().sendKeys(excelReader.getStringData("Login", 1, 1));
        clickASAP(loginPage.getLoginButton());
    }
...
//        driver.get("https://demoqa.com/");
    ...
    public void loginWithInvalidCredentials() throws InterruptedException {
        goToLoginPage();
        loginPage.getUsernamePlaceholder().sendKeys(excelReader.getStringData("Login", 1, 2));
        loginPage.getPasswordPlaceholder().sendKeys(excelReader.getStringData("Login", 1, 3));
        clickASAP(loginPage.getLoginButton());
    }
    ...
    public WebElement slucaj(int x) throws InterruptedException {
        WebElement element = null;
        for (int i = 0; i < 5; i++) {
            if (x == 5)
                if (loginPage.getLoginButton() != null)
                    element = loginPage.getLoginButton();
            Thread.sleep(200);
        }
        return element;
    }
    ...
    public void testLoginWithInvalidCredentials() throws InterruptedException {
        ...
        Assert.assertTrue(isElementDisplayed(loginPage.getLoginButton()));
